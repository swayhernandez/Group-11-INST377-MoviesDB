...
.get(async (req, res) => {
  try {
    ...
    const studiolist = await db.Studio.findOne({where: {studio_id: `${studioId}`}});
    ...
    console.error(error);
    res.send("Something went wrong on the /studio end or the studio_id isn't valid");
  }
})
.post(async(req, res) => {
  try {
    const {studioId} = req.params;
    const studiolist = await db.Studio.create({studio_id: `${studioId}`, studio_name: 'teststudio'});
    res.send('Studio added');
  } catch (error) {
    console.error(error);
    res.send('Something went wrong on the /studio end and unable to update studio_id');
  }
})
.delete(async(req, res) => {
  try {
    const {studioId} = req.params;
    const studiolist = await db.Studio.destroy({where: {studio_id: `${studioId}`}});
    res.send('Studio delete');
  } catch (error) {
    console.error(error);
    res.send('Something went wrong on the /studio end and unable to delete studio_id');
  }
})
router
  .route('/boop')
